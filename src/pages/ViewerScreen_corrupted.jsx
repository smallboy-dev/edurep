...
export default function ViewerScreen() {
  ...
  const { remoteStream } = useWebRTCViewer(hasStarted ? sessionId : null);
      ...
      <div className="main-container bg-dark h-screen overflow-hidden" style={{display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-dark)'}}>
        ...
        >
          ...
          <div className="space-y-4 mt-6">
            ...
            <p className="text-light opacity-60">Session Code: <span className="highlight" style={{fontWeight: 800}}>{sessionId}</span></p>
          </div>
          <button 
            onClick={() => {
              setHasStarted(true);
...
return (
  <div className="relative viewer-content-container" style={{width: '100vw', height: '100vh', background: 'black', overflow: 'hidden'}}>
    ...
    {remoteStream ? (
      ...
      />
    ) : safeSessionData.currentUrl ? (
      <iframe 
        src={safeSessionData.currentUrl} 
        style={{width: '100%', height: '100%', border: 'none'}}
        title="Presentation Content"
        allow="autoplay; fullscreen"
        key={safeSessionData.currentUrl}
      />
    ) : (
      <div className="main-container bg-dark h-full flex flex-col items-center justify-center">
          <p className="text-2xl font-bold tracking-widest text-white opacity-20">WAITING FOR CONTENT</p>
      </div>
    )}
